...
function ScrollController() {
    ...
    function touchStart() {
        _200PxScrolled = !1, _startScroll.x = _instance.currentScroll.x, _startScroll.y = _instance.currentScroll.y, _instance.dispatchEvent(ScrollController.ON_SCROLL_START)
    }

    function touchMove() {
        _instance.currentScroll.y = -_fakeDragItem.aniGetX(), _200PxScrolled === !1 && Math.abs(_instance.currentScroll.y - _startScroll.y) > 200 && (_200PxScrolled = !0, _instance.dispatchEvent(ScrollController.ON_200_PX_SCROLLED)), _instance.dispatchEvent(ScrollController.ON_SCROLL_MOVE)
    }
    ...
    function desktopHandle() {
        var speed = -1 != _scrollToSpeed ? _scrollToSpeed : _instance.scrollSpeed, ease = null != _scrollToEase ? _scrollToEase : Expo.easeOut;
        _isScrolling = !0;
        var delta = window.pageYOffset - _instance.currentScroll.y;
        Math.abs(delta) > 1e3 && _instance.dispatchEvent(ScrollController.ON_200_PX_SCROLLED), TweenMax.killTweensOf(_instance.currentScroll), TweenMax.to(_instance.currentScroll, speed, {
            ...
            roundProps: ["y"],
            ease: ease
        }), _scrollToSpeed = -1, _scrollToEase = null
    }

    ScrollController.ON_SCROLL_START = "ON_SCROLL_START", ScrollController.ON_SCROLL_MOVE = "ON_SCROLL_MOVE", ScrollController.ON_SCROLL_STOP = "ON_SCROLL_STOP", ScrollController.ON_SCROLL_EASE_STOP = "ON_SCROLL_EASE_STOP", ScrollController.ON_200_PX_SCROLLED = "ON_200_PX_SCROLLED", ScrollController.ON_OVERSCROLLING_TOP = "ON_OVERSCROLLING_TOP", ScrollController.ON_OVERSCROLLING_BOT = "ON_OVERSCROLLING_BOT";
    var _instance = new EventDispatcher, _target = null, _pageHeight = 0;
    _instance.currentScroll = {x: 0, y: 0};
    var _fakeDragItem = null, _dragBounds = null, _touchDragger = null, _isScrolling = !1, _startScroll = {
        x: 0,
        y: 0
    }, _200PxScrolled = !1;
    ScrollController.DEBUG_TOUCH_ON_DESKTOP = !1, ScrollController.DEFAULT_SCROLL_SPEED = .5, _instance.scrollSpeed = ScrollController.DEFAULT_SCROLL_SPEED, _instance.init = function (target) {
        if (_target = target, BrowserDetect.TABLET === !0 || BrowserDetect.MOBILE === !0 || ScrollController.DEBUG_TOUCH_ON_DESKTOP === !0) {
            _fakeDragItem = document.createElement("div"), _fakeDragItem.style.width = "0px", _fakeDragItem.style.height = "0px", AnimationUtils.apply(_fakeDragItem), _fakeDragItem.aniDisableRender(), _basic = new DragBasic, _basic.lockY = !0;
            var easePlugin = new DragEase;
            easePlugin.time = 2, easePlugin.distance = 1.5, easePlugin.ease = Expo.easeOut, easePlugin.onStart(touchStart), easePlugin.onMove(touchMove), easePlugin.onEnd(touchEnd);
            var bounds = new Rectangle(0, -_pageHeight + Assets.RESIZE_MANAGER.getWindowHeight(), window.innerWidth, _pageHeight);
            _dragBounds = new DragBounds(bounds), _dragBounds.easeDist = 4, _dragBounds.boundsEase = .4, _dragBounds.useItemsSize = !1, _touchDragger = TouchDragger.add(_fakeDragItem, [_basic, easePlugin, _dragBounds], [_target], !1), _touchDragger.enable3DTransform()
